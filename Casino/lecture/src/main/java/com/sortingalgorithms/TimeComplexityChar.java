...
public class TimeComplexityChar implements SortMethod{

    public int ARRAY_SIZE , NUMBER_OF_RUNS, NUMBER_OF_SPOTS, UPPER_BOUND;

    public TimeComplexityChar(int ARRAY_SIZE, int NUMBER_OF_RUNS, int NUMBER_OF_SPOTS, int UPPER_BOUND) {
    ...
    public void sort()  {
        ...
        File dataFile = new File("outputData\\CharSortingData.csv");
        ...
        for (int x = 0; x < NUMBER_OF_RUNS; x++) {
            ...
            ranGen.generateDeepCopyCharacter(ARRAY_SIZE);
...
//
            ...
            for (ArrayList<Character> allDataInt : ranGen.getAllDataCharacters()) {
...
//
            ...
            for (int i = 0; i < mS.length; i++) {
                mS[i].sort(0, mS[i].getCharArr().length - 1, mS[i].getCharArr());
            }
...
//
            ...
            for (int i = 0; i < qS.length; i++) {
                qS[i].sort(0, qS[i].getCharArr().length - 1, qS[i].getCharArr());
            }
