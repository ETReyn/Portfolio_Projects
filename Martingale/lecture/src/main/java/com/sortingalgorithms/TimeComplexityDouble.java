...
public class TimeComplexityDouble implements SortMethod{

    public int ARRAY_SIZE, NUMBER_OF_RUNS, NUMBER_OF_SPOTS, UPPER_BOUND;

    public TimeComplexityDouble(int ARRAY_SIZE, int NUMBER_OF_RUNS, int NUMBER_OF_SPOTS, int UPPER_BOUND) {
    ...
    public void sort()   {
        File dataFile = new File("outputData\\DoubleSortingData.csv");
        ...
        for (int x = 0; x < NUMBER_OF_RUNS; x++) {
            ...
            ranGen.generateDeepCopyDouble(ARRAY_SIZE);
...
//
            ...
            for (ArrayList<Double> allDataInt : ranGen.getAllDataDoubles()) {
...
//
            ...
            for (int i = 0; i < mS.length; i++) {
                mS[i].sort(0, mS[i].getDoubleArr().length - 1, mS[i].getDoubleArr());
            }
...
//
            ...
            for (int i = 0; i < qS.length; i++) {
                qS[i].sort(0, qS[i].getDoubleArr().length - 1, qS[i].getDoubleArr());
            }
