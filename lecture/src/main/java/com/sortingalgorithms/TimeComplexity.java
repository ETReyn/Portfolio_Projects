...
public class TimeComplexity implements SortMethod{

    public int ARRAY_SIZE, NUMBER_OF_RUNS, NUMBER_OF_SPOTS, UPPER_BOUND;

    public TimeComplexity(int ARRAY_SIZE, int NUMBER_OF_RUNS, int NUMBER_OF_SPOTS, int UPPER_BOUND) {
    ...
    public void sort()  {
        File dataFile = new File("outputData\\SortingData.csv");
        ...
        dataOutput.println("Run #;Collection Sort;Merge Sort;Quick Sort;Using Method calls for this run");
        for (int x = 0; x < NUMBER_OF_RUNS; x++) {
            RandomGenerator ranGen = new RandomGenerator(ARRAY_SIZE, NUMBER_OF_SPOTS, UPPER_BOUND);
            ranGen.generateDeepCopyInteger(ARRAY_SIZE);
...
//
            ...
            for (ArrayList<Integer> allDataInt : ranGen.getAllDataIntegers()) {
...
//
            ...
            for (int i = 0; i < mS.length; i++) {
                mS[i].sort(0, mS[i].getArr().length - 1, mS[i].getArr());
            }
...
//
            ...
            for (int i = 0; i < qS.length; i++) {
                qS[i].sort(0, qS[i].getArr().length - 1, qS[i].getArr());
            }
